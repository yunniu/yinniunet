...
class AgentController extends BaseController {
  ...
  //   	$this->assign('list',$list);
    ...
    public function login(){
        if(session('?proxy_id') && session('proxy_id')>0){
             $this->error("您已登录",U('Agent/Index/index'));
        }
      
        if(IS_POST){

            $verify = new Verify();
            if (!$verify->check(I('post.vertify'), "Agetn/Login")) {
            ...
            if(!empty($condition['username']) && !empty($condition['password'])){
				...
				//联表 admin表和admin_role表 查询出用户对应的信息，包含拥有的操作权限。
               	$agent_info = M('Proxys')->where($condition)->find();
                //dump($admin_info );
                //exit;
				if(is_array($agent_info)){
                    
                    session('proxy_id',$agent_info['id']);
                    session('agent_list',$agent_info['opened']);
