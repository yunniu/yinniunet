...
class IndexController extends BaseController {
    ...
    public function index(){
        // $this->pushVersion();
        $act_list = session('agent_list');
        $menu_list = $this->getRoleMenu($act_list);
//      var_dump($menu_list);
//      exit;
        $this->assign('menu_list',$menu_list);
        $agent_info = getAgentInfo(session('proxy_id'));
//      $admin_info = getAdminInfo(1);
	...
	/**
	 * @param $act_list 获取权限列表
	 * @return array
	 */
    public function getRoleMenu($act_list)
    {
    	$modules = $roleMenu = array();
    	$rs = M('system_module')->where('level>1 AND visible=1 AND mod_id>108')->order('mod_id ASC')->select();
		//var_dump($rs);

        if($act_list=='1'){
            foreach($rs as $row){
                if($row['level'] == 3){
                    $row['url'] = U("Agent/".$row['ctl']."/".$row['act']."");
            ...
            foreach($rs as $row){
                if(in_array($row['mod_id'],$act_list)){
                    $row['url'] = U("Agent/".trim($row['ctl'])."/".$row['act']."");
                    $modules[$row['parent_id']][] = $row;//子菜单分组
                }
                if($row['level'] == 2){
                    $pmenu[$row['mod_id']] = $row;//二级父菜单
...
//		var_dump($keys);
		...
		//调整代理管理权限的位置。
		$temp = $roleMenu[10];
		for($i = (count($roleMenu)-1);$i >= 3;$i--){
			$roleMenu[$i] = $roleMenu[$i-1];
			if($i == 3){
				$roleMenu[$i] = $temp;
			}
		}
