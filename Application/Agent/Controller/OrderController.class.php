...
class OrderController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        foreach($orderList as $k => $v){
            $id = $v['proxys_id'];
            $orderList[$k]['proxys_name'] = M('proxys')->where("id = $id")->field("name")->select()[0]['name'];
        }
    ...
    */
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            $update['discount'] = I('post.discount') ? I('post.discount') : 0;
    ...
    public function pay_cancel($order_id){
    	...
    	if(I('remark')){
    		...
    		if($data['refundType'] == 0 && $data['amount']>0){
                //修改用户账户信息
    			$result = accountLog($data['user_id'], $data['amount'], 0,  '退款到用户余额');
                //修改订单支付状态
                if($result){
                    $data2['pay_status'] = 0;
                    D('order')->where("order_id = $order_id")->save($data2);
                }
    		}
    		$orderLogic = new OrderLogic();
    		$d = $orderLogic->orderActionLog($data['order_id'],'pay_cancel',$data['remark'].':'.$note[$data['refundType']]);
    		if($d){
    			exit("<script>window.parent.pay_callback(1,$order_id);</script>");
    		}else{
    ...
    public function ajax_return_list(){
        ...
        foreach($list as $k => $v){
            $order_id = $v['order_id'];
            $proxys_id = D('order')->where("order_id = $order_id")->field("proxys_id")->select()[0]['proxys_id'];
            $list[$k]['proxys_name'] = D('proxys')->field("name")->where("id = $proxys_id")->select()[0]['name'];
        }
   ...
   {                
    ...
    public function add_order()
    {
        ...
        $payment_list = M('plugin')->where(array('status'=>1,'type'=>'payment'))->select();
        //  获取所有代理人
        $proxys_list = M('proxys')->field("id,name")->select();
        if(IS_POST)
        {
           ...
           $order['order_amount']   = $result['result']['order_amount']; // 应付金额
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
           $order['proxys_id'] = I('post.proxys_id');
...
//        exit;
        ...
        $this->assign('city',$city);
        $this->assign('area',$area);        
        $this->assign('proxys_list',$proxys_list);
        $this->display();
    }
    ...
    public function get_order_goods($order_id){
        $orderGoods = D('order_goods')->where("order_id = $order_id")->select();
        $this->assign('orderGoods',$orderGoods);
