...
class SystemController extends BaseController{
	...
	{          
		/*配置列表，配置头信息*/
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置','sms'=>'短信设置');
		$this->assign('group_list',$group_list);

		//设置显示哪一个页面，默认为shop_info.html
	...
	{
		$modules = array();
		$rs = D('system_module')->order('mod_id ASC')->select();//获取所有的modules
		if(is_array($rs)){
			foreach($rs as $row){
				$modules[$row['mod_id']] = $row;//以mod_id为索引整合modules信息
			}
			//var_dump($modules);
		}
		if($modules){
			$tree = array();
			//遍历module为top的添加到$tree,以mod_id为索引。
			foreach($modules as $k=>$v){
				if($v['module'] == 'top'){
					$tree[$k] = $v;
				}
			}
			//遍历module 等于menu的添加到$tree[$v['parent_id']]['menu']，以mod_id为索引。
			foreach($modules as $k=>$v){
				if($v['module'] == 'menu'){
					$tree[$v['parent_id']]['menu'][$k] = $v;
				}
			}
			//遍历module 等于module的添加到$tree，类上。
			foreach($modules as $k=>$v){
				if($v['module'] == 'module'){
	...
	public function cleanCache(){
		...
		if(delFile(RUNTIME_PATH))// 删除缓存 删除 \Application\Runtime 下面的所有文件
			$this->success("清除成功!!!",U('index/welcome'),1);
		else
			$this->error("操作完成!!");
	}
}