...
class UserController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        I('username') ? $condition['username'] = I('username') : false;
    ...
    public function addUser(){
        ...
        if(count($data) == 8){
            ...
            //$data['birthday'] = time($data['birthday']);
            $result = $userModel->data($data)->add();
            if($result){
                $this->success('添加成功',U('index'),1);
            }else{
                $this->error('添加失败',U('addUser'),3);
            }
            exit;
        }
        $this->display();
    ...
    public function getLevel(){
        $levelModel = D('user_level');
        $level_list = $levelModel->field("level_id,level_name")->select();
